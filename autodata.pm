...
# //////////////////////////////////////////////////
{
	...
	#30
		...
		'Проверьте правильность введёного номера SMS',
	];
}
	...
	return {
		1 => [ 
			...
			{ 
				...
				optional => 1,
				comment => 'Предоставляется с целью показать визовую историю/наличие ранее сданных отпечатков пальцев. Необходимо загрузить все страницы одним файлом.',
			},
			{
				id => 1004,
			...
			},
			{
				...
				multiple => 10,
				comment => 'По этой ссылке можно рассчитать сумму необходимых денежных средств.',
				help => {
					base => {
						ru => '/finansovaya-garantiya-4/',
						en => '/financial-guarantees-4/',
						it => '/garanzie-finanziarie-4a/',
					},
				},
			},
			{
				...
				multiple => 5,
				comment => 'Для граждан РФ: страница с личными данными и прописка.<br><br>Для иностранных граждан: регистрация/РВП/ВНЖ, действующая виза РФ, Миграционная карта/патент/разрешение на работу.',
			},
			{
				id => 1008,
			...
			},
			{
				...
				multiple => 2,
				comment => 'Паспорт или иной документ, удостоверяющий личность представителя организации (с личными данными и его подписью).',
			},
			{
				id => 1010,
				title => 'Выписка из Торговой Палаты',
				optional => 1,
				comment => 'Необходимо загрудить все страницы одним файлом.',
			},
			{
				id => 1011,
				...
				multiple => 10,
				optional => 1,
				comment => 'Необходимо предоставить, если точный адрес не указан в приглашении от организации.',
			},
			{ 
				id => 1012,
				title => 'Медицинская страховка',
				comment => 'По этой ссылке можно оформить страховой полис.',
				help => {
					base => {
			...
			{ 
				...
				optional => 1,
				multiple => 10,
				comment => 'По этой ссылке можно ознакомиться с подробной информацией о льготных категориях.',
				help => {
					base => {
			...
			},
			{
				...
				optional => 1,
				multiple => 3,
				comment => 'Допускается подача за родителей, внуков, супругов. Нотариально заверенная копия документов, подтверждающих родство.',
			},
		],
		15 => [ 
			{ 
			...
			{
				id => 15011,
				title => 'Приглашение от транспортной компании',
				multiple => 2,
			},
			{
				id => 15012,
		...
		2 => [ 
			...
			{ 
				...
				multiple => 3,
				comment => 'Минимум 30% от стоимости курса лечения или гарантия оплатить расходов государственным учреждением/организацией.',
			},
			{ 
				id => 2008,
		...
		10 => [ 
			...
			{ 
				id => 10005,
				title => 'Заявление установленного образца с указанием причины подачи документов на повторный въезд',
				comment => 'По ссылке образец заявления.',
				help => {
					base => {
			...
			{ 
				...
				multiple => 2,
				optional => 1,
				comment => 'Если Ricevuta не была проштампована при выезде',
			},
			{ 
				id => 10007,
			...
			{ 
				id => 10008,
				title => 'Копия справки из полиции с переводом',
				multiple => 5,
				optional => 1,
			...
			{
				id => 10009,
				title => 'Копия документа с указанием даты выезда',
				multiple => 3,
...
# //////////////////////////////////////////////////
{
	return {
		1 => {
			1 => 'Уважаемый заявитель!<br>Вы начали создание записи на сайте <b>Визового центра</b>.<br>Вы в любой момент можете продолжить запись перейдя по ссылке:',
			...
			7 => 'С уважением,<br>Визовый центр',
		},
		2 => {
			1 => 'Уважаемый заявитель!<br>Вы начали оформление записи на услугу <b>«Онлайн проверка документов»</b> на сайте <b>Визового центра</b>.<br>Вы в любой момент, в течение 14 дней, Вы можете продолжить запись перейдя по ссылке:',
			2 => '<br><br><a href="/autoform/?t=[token]" style="display: inline-block; font-family: arial,sans-serif; font-size: 14px; font-weight: bold; color: #444444; text-decoration: none; user-select: none; padding: .2em 1.2em; outline: none; border: 1px solid #c0c0c0; border-radius: 2px; background: #F5F5F5; target="_blank">',
			3 => 'Информация о записи',
			4 => '</a><br><br>',
...
# //////////////////////////////////////////////////
{
	return {
		1 => {
			...
			6 => 'С уважением,<br>Визовый центр',
		},
		2 => {
			...
			8 => 'С уважением,<br>Визовый центр',
		},
		3 => {
